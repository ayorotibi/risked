...
# Step 4: Bayesian Network & Inference (pyAgrum)
...
# --- pyAgrum Bayes Net construction ---
bn = gum.BayesNet("RiskBN")

# Add binary variables with labels "0" and "1"
for n in nodes:
    var = gum.LabelizedVariable(n, n, ["0", "1"])
    bn.add(var)

# Add arcs
for (p, c) in edges:
    if not bn.existsArc(p, c):
        bn.addArc(p, c)

# Helper to fill CPTs for conditional nodes

def set_cpt_from_rows(bn, node, parents, child_rows):
    k = len(parents)
    needed_cols = 2 ** k

    cols = []
    for _, r in child_rows.iterrows():
        p1 = float(r['posterior'])
        p0 = 1.0 - p1
        cols.append((p0, p1))

    while len(cols) < needed_cols:
        cols.append((0.5, 0.5))

    cols = cols[:needed_cols]

    cpt = bn.cpt(node)

    # Fill using Instantiation iteration order, tracking parent configurations
    inst = gum.Instantiation(cpt)
    inst.setFirst()
    parent_config_idx = -1
    last_parent_values = None
    current_p0, current_p1 = (0.5, 0.5)

    while not inst.end():
        cur_parent_values = tuple(inst.val(p) for p in parents) if parents else ()
        if cur_parent_values != last_parent_values:
            last_parent_values = cur_parent_values
            parent_config_idx += 1
            if 0 <= parent_config_idx < needed_cols:
                current_p0, current_p1 = cols[parent_config_idx]
            else:
                current_p0, current_p1 = (0.5, 0.5)

        node_state = inst.val(node)
        cpt[inst] = current_p0 if node_state == 0 else current_p1
        inst.inc()
...
for node in nodes:
    ...
    if not parents:
        ...
        if not prob_row.empty:
            ...
            p0, p1 = 0.5, 0.5
        bn.cpt(node).fillWith([p0, p1])
    else:
        set_cpt_from_rows(bn, node, parents, child_rows)

progress4.progress(40)
status4.text("Bayesian Network constructed.")

# --- Inference (pyAgrum) ---
# Root / leaf selection logic as before
...
ie.makeInference()
root_posterior = ie.posterior(root)

ie2 = gum.LazyPropagation(bn)
ie2.setEvidence({root: 1})
ie2.makeInference()
leaf_given_root1 = ie2.posterior(leaf)

progress4.progress(100)
status4.text("Inference completed.")

# Step 5: Single Node Sensitivity Analysis
# Onefold Sensitivity Analysis
...
def onefold_query(node, bn, root):
    result_dict = {'node': node}

    # What-if: node fails (0)
    try:
        ie0 = gum.LazyPropagation(bn)
        ie0.setEvidence({node: 0})
        ie0.makeInference()
        result_dict['root_prob_given_node0'] = float(ie0.posterior(root)[1])
    except Exception:
    ...
    try:
        ...
        ie1.setEvidence({node: 1})
        ie1.makeInference()
        result_dict['root_prob_given_node1'] = float(ie1.posterior(root)[1])
    except Exception:
...
for i in range(0, len(analysis_nodes), chunk_size):
    ...
    chunk_results = Parallel(n_jobs=-1)(
        delayed(onefold_query)(node, bn, root) for node in chunk
    )
...
def multi_node_sensitivity_analysis_baseline(bn, nodes, root_node):
        ...
        for r in range(1, n + 1):
            for node_subset in itertools.combinations(selected_nodes, r):
                ...
                try:
                    ...
                    ie.makeInference()
                    root_prob = float(ie.posterior(root_node)[1])
                except Exception:
                    root_prob = None
...
multi_node_sensitivity_analysis_baseline(bn, nodes, root)
